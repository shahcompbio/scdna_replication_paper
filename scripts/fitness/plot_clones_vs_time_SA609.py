...
def get_args():
    ...
    p.add_argument('treated_input', type=str, help='Table of the number of cells per cell cycle phase and clone for the 1st treated sample')
    p.add_argument('untreated_input', type=str, help='Table of the number of cells per cell cycle phase and clone for the 1st untreated sample')
    p.add_argument('treated_input2', type=str, help='Table of the number of cells per cell cycle phase and clone for the 2nd treated sample')
    p.add_argument('untreated_input2', type=str, help='Table of the number of cells per cell cycle phase and clone for the 2nd untreated sample')
...
def plot_clone_stackplots(df_U, df_T, df_U2, df_T2, argv):
    """
    ...
    timepoints, clone_frac_g_vs_time, clone_frac_s_vs_time, clone_legend = compute_clone_fracs_vs_time(df_U2)
    ...
    ax[4].stackplot(timepoints, clone_frac_g_vs_time, labels=clone_legend)
    ax[5].stackplot(timepoints, clone_frac_s_vs_time, labels=clone_legend)
    ...
    timepoints, clone_frac_g_vs_time, clone_frac_s_vs_time, clone_legend = compute_clone_fracs_vs_time(df_T2)
    ax[6].stackplot(timepoints, clone_frac_g_vs_time, labels=clone_legend)
    ax[7].stackplot(timepoints, clone_frac_s_vs_time, labels=clone_legend)
...
def plot_total_cell_count_stackplot(df_U, df_T, df_U2, df_T2, argv):
    """
    ...
    # second untreated sample
    timepoints, total_counts, phase_legend = compute_total_cell_count_vs_time(df_U2)
...
def main():
    ...
    df_T2 = pd.read_csv(argv.treated_input2, sep='\t')
    df_U2 = pd.read_csv(argv.untreated_input2, sep='\t')

    # sort timepoints based on the timepoint_int column
    df_T = sort_timepoints(df_T)
    df_U = sort_timepoints(df_U)
    df_T2 = sort_timepoints(df_T2)
    df_U2 = sort_timepoints(df_U2)

    # find the 3 earliest timepoints in the 1st untreated sample as they are the ancestors for all other samples
    ...
    clone_list = list(set(df_U.clone_id.unique()).union(set(df_T.clone_id.unique()).union(set(df_U2.clone_id.unique()).union(set(df_T2.clone_id.unique())))))
    # fill in missing clones with 0s
    df_T = fill_in_missing_clones(df_T, clone_list)
    df_U = fill_in_missing_clones(df_U, clone_list)
    df_T2 = fill_in_missing_clones(df_T2, clone_list)
    df_U2 = fill_in_missing_clones(df_U2, clone_list)
    ...
    plot_clone_stackplots(df_U, df_T, df_U2, df_T2, argv)
    ...
    plot_total_cell_count_stackplot(df_U, df_T, df_U2, df_T2, argv)
