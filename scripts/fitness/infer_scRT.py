...
def main():
    ...
    temp_cn_g = cn_g[['cell_id', 'chr', 'start', 'end', argv.gc_col, argv.cn_col, 'library_id', 'clone_id', argv.input_col]]

    print('creating scrt object')
    # create SPF object with input
    scrt = scRT(temp_cn_s, temp_cn_g, input_col=argv.input_col, rt_prior_col=None, assign_col=argv.input_col,
    ...
    # save output files
    cn_s_with_scrt.to_csv(argv.cn_s_out, sep='\t', index=False)
