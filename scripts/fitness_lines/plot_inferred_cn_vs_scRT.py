...
def plot_frac_rt_distributions(df, argv):
    df_frac = df[['cell_id', argv.frac_rt_col, 'timepoint', 'clone_id']].drop_duplicates().reset_index(drop=True)
    ...
    sns.histplot(data=df_frac, x=argv.frac_rt_col, hue='timepoint', multiple='stack', ax=ax[1])
