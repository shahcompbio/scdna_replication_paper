...
def get_args():
    ...
    p.add_argument('cn_l_input', help='input long-form copy number dataframe for low quality cells')
    ...
    p.add_argument('cn_l_out', help='output tsv that is same as cn_l_input with timepoint info added')
...
def main():
    ...
    cn_l = pd.read_csv(argv.cn_l_input, sep='\t')
    time_df = pd.read_csv(argv.time_legend)

    # get rid of useless columns in the time_df
    ...
    if 'library_id' not in cn_l.columns:
        cn_l['library_id'] = cn_l['cell_id'].apply(lambda x: x.split('-')[1])
    ...
    cn_l_out = pd.merge(cn_l, time_df)

    # save the outpt now that times have been added
    cn_s_out.to_csv(argv.cn_s_out, sep='\t', index=False)
    cn_g_out.to_csv(argv.cn_g_out, sep='\t', index=False)
    cn_l_out.to_csv(argv.cn_l_out, sep='\t', index=False)
