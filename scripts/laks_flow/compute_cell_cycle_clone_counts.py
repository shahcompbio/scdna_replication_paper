...
def get_args():
    ...
    p.add_argument('cn_s_T47D', type=str, help='full df for T47D S-phase cells')
    p.add_argument('cn_g_T47D', type=str, help='full df for T47D G1/2-phase cells')
    p.add_argument('cn_s_GM18507', type=str, help='full df for GM18507 S-phase cells')
    p.add_argument('cn_g_GM18507', type=str, help='full df for GM18507 G1/2-phase cells')
    p.add_argument('output', type=str, help='Table of the number of cells per cell cycle phase and clone')
...
def clone_spf_analysis(cn_s, cn_g):
...
def main():
    ...
    cn_s_T47D = pd.read_csv(argv.cn_s_T47D, sep='\t')
    cn_g_T47D = pd.read_csv(argv.cn_g_T47D, sep='\t')
    cn_s_GM18507 = pd.read_csv(argv.cn_s_GM18507, sep='\t')
    cn_g_GM18507 = pd.read_csv(argv.cn_g_GM18507, sep='\t')

    # compute the number of cells in each clone for each cell cycle phase
    # use a hypergeometric test to see which clones are enriched/depleted for S-phase cells
    counts_T47D = clone_spf_analysis(cn_s_T47D, cn_g_T47D)
    counts_GM18507 = clone_spf_analysis(cn_s_GM18507, cn_g_GM18507)

    # merge the counts from the two datasets into one table
    counts = []
    for d, temp_counts in zip(['T47D', 'GM18507'], [counts_T47D, counts_GM18507]):
