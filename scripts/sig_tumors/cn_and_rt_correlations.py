...
def get_args():
    ...
    p.add_argument('-src', '--sample_rt_corrs', type=str, help='csv of sample RT correlations')
    p.add_argument('-scd', '--sample_cn_dists', type=str, help='csv of sample CN distances')
    p.add_argument('-crc', '--clone_rt_corrs', type=str, help='csv of clone RT correlations')
    p.add_argument('-ccd', '--clone_cn_dists', type=str, help='csv of clone CN distances')
...
def plot_sample_corrs(cn, rt, sample_cn_cols, sample_rt_cols, argv):
    ...
    # save the distance matrix and correlation matrix to csv files
    cn_dist.to_csv(argv.sample_cn_dists)
    rt_corrs.to_csv(argv.sample_rt_corrs)
...
def plot_clone_corrs(cn, rt, clone_cn_cols, clone_rt_cols, argv):
    ...
    # save the distance and correlation matrices to csv files
    cn_dist.to_csv(argv.clone_cn_dists)
    rt_corrs.to_csv(argv.clone_rt_corrs)
