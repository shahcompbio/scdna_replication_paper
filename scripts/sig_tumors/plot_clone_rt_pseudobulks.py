...
from scdna_replication_tools.plot_utils import get_clone_cmap, plot_cell_cn_profile2
...
def get_args():
    ...
    p.add_argument('counts', type=str, help='table of cell counts for each clone')
...
def plot_clone_rt_profiles(rt, counts, argv):
    ...
    for clone_id, col in zip(clones, cols):
        # get the number of cells in this clone
        if clone_id in counts['clone_id'].values:
            num_s_cells = sum(counts[counts['clone_id'] == clone_id]['num_cells_s'].values)
        else:
            num_s_cells = 0
        label = '{} (n={})'.format(clone_id, int(num_s_cells))
...
def main():
    ...
    # load cell counts for each clone
    ...
    plot_clone_rt_profiles(rt, counts, argv)
