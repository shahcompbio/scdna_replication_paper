...
# Define a custom function that calculates the frequency of the most common value
def most_common_freq(x):
    return x.value_counts().max() / x.value_counts().sum()
...
def main():
    ...
    clone_cns = compute_consensus_clone_profiles(df, argv.cn_state_col, cn_state_col=argv.cn_state_col, clone_col='clone_id', aggfunc=most_common_freq)
    ...
    sample_cns = compute_consensus_clone_profiles(df, argv.cn_state_col, cn_state_col=argv.cn_state_col, clone_col='sample_id', aggfunc=most_common_freq)
    ...
    if argv.dataset not in df['sample_id'].unique():
        ...
        dataset_cns = compute_consensus_clone_profiles(df, argv.cn_state_col, cn_state_col=argv.cn_state_col, clone_col='dataset', aggfunc=most_common_freq)
