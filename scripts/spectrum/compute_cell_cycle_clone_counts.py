...
def main():
    ...
    df = clone_spf_analysis(cn_s, cn_g, argv)

    # place a dummy column for library_id since we want to merge all libraries together when computing clone counts
    cn_s['library_id'] = 'all'
    cn_g['library_id'] = 'all'

    # plot S-phase fractions at the clone and sample levels
    # save both the plot and table used to make the plot
    df2 = clone_spf_analysis(cn_s, cn_g, argv)

    # concatenate the two tables where one has library_id='all' and the other has the actual library IDs
