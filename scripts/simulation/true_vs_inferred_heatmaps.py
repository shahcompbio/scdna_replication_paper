...
def plot_true_vs_inferred_heatmaps(df, argv):
    ...
    accuracy = 1.0 - (sum(abs(df[argv.true_rep_state] - df[argv.model_rep_state])) / df.shape[0])
    ...
    if len(clone_dict) > 1:
        ...
        ax = fig.add_axes([0.0, 0.0, 0.04, 0.25])
